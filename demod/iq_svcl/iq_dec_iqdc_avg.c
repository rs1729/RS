...
static int f32read_cblock(dsp_t *dsp) {
    ...
    for (n = 0; n < len; n++) {
        ...
        float xn = x-IQdc.avgIQx;
        float yn = y-IQdc.avgIQy;
        dsp->decMbuf[n] = xn + I*yn;

        if (dsp->opt_iqdc == 1) // default: without buffer, but updates only every IQdc.maxcnt
        {
            IQdc.sumIQx += x;
            IQdc.sumIQy += y;
            IQdc.cnt += 1;
        ...
        else if (dsp->opt_iqdc == 2) // moving average length IQdc.maxlim-1 , circular buffer
        {
            IQdc.buf[IQdc.cnt] = x + I*y;
            //IQdc.avgIQ += (IQdc.buf[IQdc.cnt] - IQdc.buf[(IQdc.cnt+1)%IQdc.maxlim])/(float)IQdc.maxlim;
            IQdc.avgIQ += (IQdc.buf[IQdc.cnt] - IQdc.buf[IQdc.cnt+1])/(float)(IQdc.maxlim-1);
            IQdc.avgIQx = creal(IQdc.avgIQ);
            IQdc.avgIQy = cimag(IQdc.avgIQ);
            IQdc.cnt += 1;
            if (IQdc.cnt == IQdc.maxlim) {
                IQdc.cnt = 0;
                IQdc.buf[IQdc.maxlim] = IQdc.buf[IQdc.maxlim-1]; // duplicate before roll-over
            }   
        }
        else if (dsp->opt_iqdc == 3) // exponential
        {
            float _r = 1.0f/(float)IQdc.maxlim; // 1e-5
            IQdc.avgIQ += _r * dsp->decMbuf[n];
            IQdc.avgIQx = creal(IQdc.avgIQ);
            IQdc.avgIQy = cimag(IQdc.avgIQ);
        }
   }
...
static int init_buffers(dsp_t *dsp) {
    ...
    if (dsp->opt_iqdc == 2) {
        IQdc.buf = calloc(IQdc.maxlim + 1, sizeof(float complex)); if (IQdc.buf == NULL) return -1;
    }
...
static int free_buffers(dsp_t *dsp) {
    ...
    if (dsp->opt_iqdc == 2)
    {
        if (IQdc.buf) { free(IQdc.buf); IQdc.buf = NULL; }
    }
...
int main(int argc, char *argv[]) {
    ...
    while ((*argv) && (!wavloaded)) {
        ...
        else if   (strcmp(*argv, "--iqdc") == 0) { // iq-dc removal
            int _d = 0;
            ++argv;
            if (*argv) _d = atoi(*argv);
            else return -1;
            if (_d >= 0 && _d < 4) option_iqdc = _d; // default: 1
        }
    ...
    dsp.opt_iqdc = option_iqdc; // f32read_cblock()
