...
static int f32soft_read(FILE *fp, float *s) {
    unsigned int word = 0;
    short *b = (short*)&word;
    float *f = (float*)&word;
    int bps = 32;
...
static int find_softbinhead(FILE *fp, hdb_t *hdb, float *score) {
    int headlen = hdb->len;
    float sbit;
    float mv;
...
int main(int argc, char *argv[]) {
    ...
    while ((*argv) && (!wavloaded)) {
        ...
        else if   (strcmp(*argv, "--soft") == 0) { option_softin = 1; }  // float32 soft input
                ...
                while ( byte_count < FRAME_LEN )
                {
                    if (option_bin) {
                        ...
                        if (option_iq >= 2) {
                            ...
                            bitQ = read_slbit(&dsp, &bit, 0, bitofs, bitpos, bl, 0);
                        }
                        else {
                            bitQ = read_slbit(&dsp, &bit, 0, bitofs, bitpos, -1, 0);
                        }
                    }
