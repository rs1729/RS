...
/*
    gcc rs92agp_ecc.c -lm -o rs92agp_ecc
    (includes nav_gps_vel.c, bch_ecc.c)
...
rscfg_t cfg_rs92agp = { 92, 230, 8, 238, 8, 300};
...
#define BLOCK_GPS 0x673D  // 0x6c3d   frame[posGPS_TOW-2], frame[posGPS_TOW-1]
#define BLOCK_AUX 0x6812  // sgp: 0x6805
...
int rs92agp_ecc(int msglen) {
    ...
    for (i = 0; i < rs_R;               i++) cw[i]      = frame[cfg_rs92agp.parpos+i];
    for (i = 0; i < cfg_rs92agp.msglen; i++) cw[rs_R+i] = frame[cfg_rs92agp.msgpos+i];

    errors = rs_decode(cw, err_pos, err_val);

    //for (i = 0; i < cfg_rs92agp.hdrlen; i++) frame[i] = data[i];
    for (i = 0; i < rs_R;               i++) frame[cfg_rs92agp.parpos+i] = cw[i];
    for (i = 0; i < cfg_rs92agp.msglen; i++) frame[cfg_rs92agp.msgpos+i] = cw[rs_R+i];
